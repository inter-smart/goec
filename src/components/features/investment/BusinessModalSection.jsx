      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            setActiveId(Number(entry.target.id)); // update active
          }
        });
      },
      {
    ...
    <section className="w-full h-auto block py-[40px] sm:py-[60px] xl:py-[100px] 2xl:py-[120px]">
      <div className="container">
        <Heading
          as="h2"
          size="heading2"
          className="text-[#030303] mb-[30px] sm:mb-[40px] xl:mb-[60px] 2xl:mb-[80px]"
        >
          {data?.title}
        </Heading>
        <div className="flex flex-wrap mx-[-15px] sm:mx-[-20px] xl:mx-[-25px] 2xl:mx-[-30px] [&>*]:px-[15px] sm:[&>*]:px-[20px] xl:[&>*]:px-[25px] 2xl:[&>*]:px-[30px]">
          <div className="w-full h-full sm:w-1/2 xl:w-[300px] 2xl:w-[368px] sticky top-[var(--header-y)]">
          ...
          <div className="w-full sm:w-1/2 xl:w-[calc(100%-300px)] 2xl:w-[calc(100%-368px)]">
            {data?.item_business?.map((item, index) => (
              <div
              ...
              >
                ...
                >
                  ...
                  <div className="w-full h-auto p-[15px_10px] xl:p-[35px_40px] 2xl:p-[40px_50px]">
                    <Heading
                      as="h3"
                      size="heading3"
                      className="font-medium text-[#030303] mb-[15px] xl:mb-[20px] 2xl:mb-[30px]"
                    >
                      {item?.title}
                    </Heading>
                    <div>
                      <div
                        className="typography [&_ul]:flex [&_ul]:flex-wrap [&_ul]:justify-between [&_ul>li]:w-[45%] [&_ul>li]:list-none [&_ul>li]:relative [&_ul>li]:before:content-[''] [&_ul>li:before]:bg-[url('/images/icon-list.svg')] [&_ul>li]:before:w-[15px] xl:[&_ul>li]:before:w-[20px] 2xl:[&_ul>li]:before:w-[25px] [&_ul>li]:before:aspect-square [&_ul>li]:before:block [&_ul>li]:before:absolute [&_ul>li]:before:top-[8px] [&_ul>li]:before:left-[-30px] [&_ul>li]:before:bg-no-repeat [&_ul>li]:before:bg-cover [&_ul>li]:before:bg-center bg-ima"
